...
# tauD sweep data -------------------------------------------------
tauD_frame <- Parameter_sweep_load_data(treatments_filename = "Treatments_master.csv",
                                           simulation_estimates_folder = "data/tauD",
                                           new_zero_value = 1e-8, 
                                           parameter_for_xaxis_values = 'tauD')
tauD_mean_frame <- Parameter_sweep_mean_calculation(treatments_filename = "Treatments_master.csv",
                                                       simulation_estimates_folder = "data/tauD",
                                                       new_zero_value = 1e-8, 
                                                       parameter_for_xaxis_values = 'tauD')
tauD <- Gamma_box_plot(sim_data_frame = tauD_frame, 
                          mean_dataframe = tauD_mean_frame,
                          x_axis_label = expression(paste("Plasmid loss rate (", ' hr'^-1, ")")),
...
save_plot("SI_Fig4.pdf", plot = Figure, base_width = Figure_width, base_height = Figure_height)
