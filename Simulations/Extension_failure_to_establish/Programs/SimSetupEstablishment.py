...
for i in range(len(df['Treatment_ID'])):
    ...
    for e in range(df['Num_of_experiments'][i]):
        ...
        # write lines for the extra simulation with only transconjugants
        for a in range(df['Num_sims_I3'][i]):
            incubation = "I3" # used to be IT for transconjugant only incubation
            shell_line (SimRun_path, df, f, i, e, incubation, a)
