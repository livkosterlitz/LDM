...
def gamma_variation_function(param, std_dev):  
    import math
    from scipy.stats import norm
    dist_mean = math.log10(param)
    sampled_param = norm.rvs(loc = dist_mean, scale = std_dev, size = 1)[0]
    set_param = 10**sampled_param
    return(set_param)

def psi_variation_function(param, std_dev):
    import math
    from scipy.stats import norm
    dist_mean = math.log(param)
    sampled_param = norm.rvs(loc = dist_mean, scale = std_dev, size = 1)[0]
    set_param = math.exp(sampled_param)
    return(set_param)

def shell_line_variation (SimRun_path, df, f, i, e, incubation, replicate):
    treatment = str(df['Treatment_ID'][i])
    f.write('time' + ' ')
    f.write('python3' + ' ')
    f.write(SimRun_path + ' ')
    f.write('--output_filename ' + output_name(treatment, e, incubation, replicate) + ' ')
    f.write('--psiD ' + str(psi_variation_function(df['psiD'][i], df['std_dev'][i])) + ' ')
    f.write('--psiR ' + str(psi_variation_function(df['psiR'][i], df['std_dev'][i])) + ' ')
    f.write('--psiT ' + str(psi_variation_function(df['psiT'][i], df['std_dev'][i])) + ' ')
    f.write('--psiF ' + str(psi_variation_function(df['psiF'][i], df['std_dev'][i])) + ' ')
    f.write('--gammaDR ' + str(gamma_variation_function(df['gammaDR'][i], df['std_dev'][i])) + ' ')
    f.write('--gammaTR ' + str(gamma_variation_function(df['gammaTR'][i], df['std_dev'][i])) + ' ')
    f.write('--gammaDF ' + str(gamma_variation_function(df['gammaDF'][i], df['std_dev'][i])) + ' ')
    f.write('--gammaTF ' + str(gamma_variation_function(df['gammaTF'][i], df['std_dev'][i])) + ' ')
    f.write('--tauD ' + str(df['tauD'][i]) + ' ')
    f.write('--tauT ' + str(df['tauT'][i]) + ' ')
    if incubation == 'I3': 
        f.write('--D_0 ' + str(0) + ' ')
...
def shell_line_growthonly_variation (SimRun_path, df, f, i, e, incubation, replicate):
    treatment = str(df['Treatment_ID'][i])
    f.write('time' + ' ')
    f.write('python3' + ' ')
    f.write(SimRun_path + ' ')
...
def shell_line_transferonly_variation (SimRun_path, df, f, i, e, incubation, replicate):
    treatment = str(df['Treatment_ID'][i])
    f.write('time' + ' ')
    f.write('python3' + ' ')
    f.write(SimRun_path + ' ')
